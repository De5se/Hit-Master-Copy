...
namespace UnityEditor.AI
{
    public class NavMeshAssetManager : ScriptableSingleton<NavMeshAssetManager>
    {
        ...
        List<AsyncBakeOperation> mBakeOperations = new List<AsyncBakeOperation>();
        internal List<AsyncBakeOperation> GetBakeOperations() { return mBakeOperations; }
        ...
        List<SavedPrefabNavMeshData> mPrefabNavMeshDataAssets = new List<SavedPrefabNavMeshData>();
        ...
        {
            foreach (var oper in mBakeOperations)
            {
            ...
            mBakeOperations.RemoveAll(o => o.BakeOperation == null || o.BakeOperation.isDone);
            if (mBakeOperations.Count == 0)
                EditorApplication.update -= UpdateAsyncBuildOperations;
        }
        ...
        void StoreNavMeshDataIfInPrefab(NavMeshSurface surfaceToStore)
        {
            ...
            foreach (var storedAssetInfo in mPrefabNavMeshDataAssets)
                if (storedAssetInfo.Surface == surfaceToStore)
                    return;

            if (mPrefabNavMeshDataAssets.Count == 0)
            {
            ...
            mPrefabNavMeshDataAssets.Add(new SavedPrefabNavMeshData { Surface = surfaceToStore, NavMeshData = isDataOwner ? surfaceToStore.navMeshData : null });
        }
        ...
        {
            for (var i = mPrefabNavMeshDataAssets.Count - 1; i >= 0; i--)
            {
                var storedAssetInfo = mPrefabNavMeshDataAssets[i];
                if (storedAssetInfo.Surface == surface)
                {
                    ...
                    mPrefabNavMeshDataAssets.RemoveAt(i);
                    break;
                }
            }
        ...
        void ForgetUnsavedNavMeshDataChanges(PrefabStage prefabStage)
        {
            ...
            do
            {
                ...
                for (var i = mPrefabNavMeshDataAssets.Count - 1; i >= 0; i--)
                {
                    var storedPrefabInfo = mPrefabNavMeshDataAssets[i];
